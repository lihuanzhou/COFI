# -*- coding: utf-8 -*-
"""03.WRI COFI_Cleaning_FDI_Markets.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1vMxBnMBc3xHfzxpXsCzZvJiAeWYK89uH

Load Libraries

! Note - the original FDI file is corrupted, and was made a clean version before processing in this python script
"""
...
if len(map_temp) > 1:
    appended_data = []
    print("FILES SEEN BY CLEANING fdi markets")
...
# Convert 'Project date' column to datetime format
df['Project date'] = pd.to_datetime(df['Project date'])

# Split the date into Year, Month, and Day columns
df['Year'] = df['Project date'].dt.year
df['Month'] = df['Project date'].dt.month
df['Day'] = df['Project date'].dt.day
...
for i in df.index:
    # Create a new column mapping to "province"
    df.loc[i,'province'] = region_clean(df.loc[i,'Destination state'])

    # Create a new column mapping to "city"
    df.loc[i,'city'] = region_clean(df.loc[i,'Destination city'])

    # Create a new column mapping to "primary_fuel" from Sub-sector column
    df.loc[i,'primary_fuel'] = extract_fuel_proxy(df.loc[i,'Sub-sector'])

    # Create a new column mapping to "equity_investor_amount" from Capital investment column
    # missing unit info
